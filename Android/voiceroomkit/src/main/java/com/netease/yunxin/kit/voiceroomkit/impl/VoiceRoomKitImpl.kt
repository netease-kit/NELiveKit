...
internal class VoiceRoomKitImpl : NEVoiceRoomKit, CoroutineRunner() {
        ...
        launch {
            voiceRoomHttpService.httpErrorEvents.collect { evt ->
                if (evt.code == NEErrorCode.UNAUTHORIZED ||
                    ...
                    authListeners.forEach {
                        it.onAuthEvent(NEAuthEvent.FORBIDDEN)
                    }
    ...
    override fun muteMyVideo(callback: NEVoiceRoomCallback<Unit>?) {
        VoiceRoomLog.logApi("muteMyVideo")
        myRoomService.muteMyVideo(object : NECallback2<Unit>() {
            ...
            override fun onError(code: Int, message: String?) {
                VoiceRoomLog.e(tag, "muteMyVideo onError code:$code")
                callback?.onFailure(code, message)
            }
        })
    }

    override fun unmuteMyVideo(callback: NEVoiceRoomCallback<Unit>?) {
        VoiceRoomLog.logApi("unmuteMyVideo")
        myRoomService.unmuteMyVideo(object : NECallback2<Unit>() {
            ...
            override fun onError(code: Int, message: String?) {
                VoiceRoomLog.e(tag, "unmuteMyVideo onError code:$code")
                callback?.onFailure(code, message)
            }
        })
    }
