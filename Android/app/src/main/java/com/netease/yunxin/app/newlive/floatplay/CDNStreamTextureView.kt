...
import com.netease.yunxin.app.newlive.audience.utils.PlayerVideoSizeUtils
...
class CDNStreamTextureView @JvmOverloads constructor(
    ...
    fun prepare(liveInfo: LiveInfo?) {
        val liveConfig = liveInfo?.live?.liveConfig
    ...
    fun adjustVideoSizeForPk(isPrepared: Boolean) {
        ...
        val x = width / 2f
        val y = StatusBarConfig.getStatusBarHeight(ActivityUtils.getTopActivity()) + SpUtils.dp2pix(64f
        ) + height / 2f
