...
import com.netease.yunxin.kit.common.utils.SizeUtils.dp2px
...
class LinkSeatsAudienceRecycleView : RecyclerView {
    ...
    fun setList(memberInfoList: List<SeatMemberInfo>) {
        mAdapter.setList(memberInfoList)
    }
    ...
    fun getItem(uuid: String): SeatMemberInfo? {
        for (info in list) {
            if (info.seatInfo.uuid == uuid) {
                return info
            }
        }
        return null
    ...
    operator fun contains(accountId: String?): Boolean {
        for (member in list) {
            if (!TextUtils.isEmpty(accountId) && accountId == member.seatInfo.uuid) {
                return true
            }
        }
        return false
    ...
    class SeatsAdapter(
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
            return SeatsViewHolder(
                LayoutInflater.from(context)
                    .inflate(R.layout.live_single_seats_layout, parent, false)
            )
        }
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            ...
            seatsViewHolder.setCloseCallback(object : SeatsViewHolder.CloseCallback {
...
//                        val params =
//                            AnchorActionSeatParams(toAccountId = member?.seatInfo?.uuid)
...
//                            })
    ...
    private class SeatsViewHolder(itemView: View) : ViewHolder(itemView) {
        private val seatsView: SingleAudienceSeatsView? = itemView.findViewById(
            R.id.audience_seats_view
        )
        ...
        fun bindData(member: SeatMemberInfo?, useScene: Int) {
            ...
            seatsView?.setCloseSeatCallback(object : SingleAudienceSeatsView.CloseSeatCallback {
                override fun closeSeat(member: SeatMemberInfo?) {
