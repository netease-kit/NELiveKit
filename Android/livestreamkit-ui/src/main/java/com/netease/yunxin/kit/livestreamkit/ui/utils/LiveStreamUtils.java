...
public class LiveStreamUtils {
  ...
  public static boolean isHost(String uuid) {
    ...
    return TextUtils.equals(member.getRole().getName(), NELiveRoomRole.HOST.getValue());
  }
  ...
  public static NERoomMember getHost() {
    ...
    for (int i = 0; i < allMemberList.size(); i++) {
      NERoomMember member = allMemberList.get(i);
      if (TextUtils.equals(member.getRole().getName(), NELiveRoomRole.HOST.getValue())) {
        return member;
      }
    }
    return null;
  ...
  public static List<LiveInfo> voiceRoomInfos2LiveInfos(List<NELiveStreamRoomInfo> voiceRoomInfos) {
    List<LiveInfo> result = new ArrayList<>();
    for (NELiveStreamRoomInfo roomInfo : voiceRoomInfos) {
      result.add(neVoiceRoomInfo2RoomInfo(roomInfo));
    }
    return result;
  }

  public static LiveInfo neVoiceRoomInfo2RoomInfo(NELiveStreamRoomInfo roomInfo) {
    if (roomInfo == null) {
      return null;
    }
