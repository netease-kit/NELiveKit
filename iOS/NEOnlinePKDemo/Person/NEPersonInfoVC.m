...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            [cell.personView.indicatorImageView sd_setImageWithURL:[NSURL URLWithString:@""] placeholderImage:[UIImage imageNamed:@"avator"]];
        }else {
...
- (void)logout {
    ...
    [[AuthorManager shareInstance] logoutWithConfirm:nil withCompletion:^(YXUserInfo * _Nullable userinfo, NSError * _Nullable error) {
        if (error == nil) {
