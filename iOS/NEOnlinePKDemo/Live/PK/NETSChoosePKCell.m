...
@property (nonatomic, strong)   NELiveDetail   *model;
...
- (void)installWithModel:(NELiveDetail *)model indexPath:(NSIndexPath *)indexPath
{
    _model = model;
    
    self.topLine.hidden = (indexPath.row == 0);
    self.avatar = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:model.anchor.avatar]]];
    self.nick.text = model.anchor.userName.length ? model.anchor.userName : model.anchor.userUuid ;
...
+ (NETSChoosePKCell *)cellWithTableView:(UITableView *)tableView indexPath:(NSIndexPath *)indexPath datas:(NSArray <NELiveDetail *> *)datas
{
