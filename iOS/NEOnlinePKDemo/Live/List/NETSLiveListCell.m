...
{
    self.roomName.text = model.live.liveTopic;
    self.anchorName.text = model.anchor.userName;
    NSURL *coveUrl = [NSURL URLWithString:model.live.cover];
    NSData* data = [NSData dataWithContentsOfURL:coveUrl];
    self.coverView.image = [UIImage imageWithData:data];
    int32_t audienceNo = MAX(model.live.audienceCount, 0);
    self.audienceNum.text = kFormatNum(audienceNo);
    
    if (model.live.live == NELiveStatusPking ||
        model.live.live == NELiveStatusPunishing ||
        model.live.live == 4) {
        self.pkView.hidden = NO;
        if (model.live.live == 4 ) {
...
+ (NETSLiveListCell *)cellWithCollectionView:(UICollectionView *)collectionView
                                   indexPath:(NSIndexPath *)indexPath
                                       datas:(NSArray <NELiveDetail *> *)datas
{
    ...
    NELiveDetail *model = datas[indexPath.row];
