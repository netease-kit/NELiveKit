...
+ (instancetype)loadConnectManageCellWithTableView:(UITableView *)tableView {
  ...
  if (!cell) {
    cell = [[NETSConnectManageCell alloc] initWithStyle:UITableViewCellStyleDefault
...
- (void)buttonClick:(UIButton *)sender {
  ...
  if ([sender isEqual:self.shutdownButton] &&
      [self.delegate respondsToSelector:@selector(didHangUpConnectAccountId:)]) {
    [self.delegate didHangUpConnectAccountId:self.userModel];
  } else if ([sender isEqual:self.videoButton] &&
             [self.delegate respondsToSelector:@selector(didCloseVideo:accountId:)]) {
    [self.delegate didCloseVideo:sender.selected accountId:self.userModel.user];
  } else if ([sender isEqual:self.micButton] &&
             [self.delegate respondsToSelector:@selector(didCloseMicrophone:accountId:)]) {
    [self.delegate didCloseMicrophone:sender.selected accountId:self.userModel.user];
  }
}
...
- (UIButton *)shutdownButton {
  if (!_shutdownButton) {
    ...
    _shutdownButton.titleLabel.font = [UIFont systemFontOfSize:14];
    _shutdownButton.layer.cornerRadius = 4;
    [_shutdownButton setGradientBackgroundWithColors:@[ HEXCOLOR(0xF359E2), HEXCOLOR(0xFF7272) ]
                                           locations:nil
                                          startPoint:CGPointMake(0, 0)
                                            endPoint:CGPointMake(1, 0)];
    [_shutdownButton addTarget:self
...
- (UIButton *)videoButton {
  if (!_videoButton) {
    ...
    [_videoButton setImage:[UIImage imageNamed:@"anchor_video_close"]
