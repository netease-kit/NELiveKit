...
- (instancetype)initWithFrame:(CGRect)frame
    gradientBackgroundWithColors:(NSArray<UIColor *> *)colors
...
- (void)setFontSize:(NSInteger)fontSize {
  ...
  for (UIView *subView in self.subviews) {
    if ([subView isMemberOfClass:[MySegmentBtn class]]) {
      ...
      btn.nameLabel.font = [UIFont systemFontOfSize:fontSize];
    }
  }
}
...
- (void)setupBottomLineView {
  ...
  if (self.bottomLineType == BottomLineTypeNormal) {
    ...
    self.lineView = [[UIView alloc]
        initWithFrame:CGRectMake(0, h - self.lineViewHeight - self.lineBottomDistanceInterval, w,
                                 self.lineViewHeight)];
    ...
    self.lineView = [[UIView alloc]
        initWithFrame:CGRectMake((w - nameLabelW) / 2,
                                 h - self.lineViewHeight - self.lineBottomDistanceInterval,
                                 nameLabelW, self.lineViewHeight)];
    self.lineView.backgroundColor = self.lineViewColor;
    ...
    self.lineView.backgroundColor = self.lineViewColor;
    self.maxLabelW = nameLabelW;
    [self.lineView cornerByRoundingCorners:UIRectCornerTopLeft | UIRectCornerTopRight
                              cornerRadius:1];
    ...
    // 统一宽度
    self.lineView = [[UIView alloc]
        initWithFrame:CGRectMake(0, h - self.lineViewHeight - self.lineBottomDistanceInterval,
                                 self.lineViewWidth, self.lineViewHeight)];
    self.lineView.backgroundColor = self.lineViewColor;
    [self addSubview:self.lineView];
  }
...
- (void)lineViewDidMoveFromIndex:(NSInteger)fromIndex
                         toIndex:(NSInteger)toIndex
                        progress:(CGFloat)progress {
  if (fromIndex < 0 || fromIndex >= self.titleArray.count || toIndex < 0 ||
...
- (CGFloat)textIsAdaptWidth:(NSString *)text {
  CGFloat w =
      [text boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, self.bounds.size.height)
