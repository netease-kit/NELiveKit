...
@interface NETSMicRequestViewController () <UITableViewDelegate,
                                            UITableViewDataSource,
                                            NETSMicRequestViewDelegate>
...
- (void)nets_getNewData {
  self.dataArray = [[self.delegate didRequestSubmitMicData] mutableCopy];
...
- (void)dealMicRequestAccept:(BOOL)isAccept userModel:(NELiveStreamSeatItem *)userModel {
  if (isAccept) {  // 接受上麦
    [self.delegate onAcceptWithSeatItem:userModel];
  } else {  // 拒绝上麦
    [self.delegate onRejectWithSeatItem:userModel];
  }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  NETSRequestConnectMicCell *requestCell =
      [NETSRequestConnectMicCell loadRequestConnectMicCellWithTableView:tableView];
