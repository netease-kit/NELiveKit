...
#import "NELiveStreamUIMicQueueView.h"
...
/// 音量信息
//@property(nonatomic, strong) NSArray<NELiveStreamMemberVolumeInfo *> *volumeInfos;
...
- (void)setGiftDatas:(NSMutableArray<NELiveStreamBatchSeatUserReward *> *)giftDatas {
...
- (void)updateWithLocalVolume:(NSInteger)volume {
  NELiveStreamMember *localMember = [NELiveStreamKit getInstance].localMember;
  // 自己讲话，单独更新
  if ([localMember.account isEqualToString:_anchorMicInfo.user]) {
    ...
    for (int i = 0; i < self.datas.count; i++) {
      NELiveStreamSeatItem *data = self.datas[i];
...
- (void)updateWithRemoteVolumeInfos:(NSArray<NELiveStreamMemberVolumeInfo *> *)volumeInfos {
  // 其他人讲话
  if ([[NELiveStreamKit getInstance].localMember.account isEqualToString:_anchorMicInfo.user]) {
    // 自己是主播
    for (int i = 0; i < self.datas.count; i++) {
      bool isIn = false;
      for (NELiveStreamMemberVolumeInfo *volume in volumeInfos) {
        NELiveStreamMember *member;
        for (NELiveStreamMember *m in [NELiveStreamKit getInstance].allMemberList) {
    ...
    for (int i = 0; i < self.datas.count; i++) {
      ...
      for (NELiveStreamMemberVolumeInfo *volume in volumeInfos) {
        ...
        NELiveStreamSeatItem *data = self.datas[i];
        if (![data.user isEqualToString:[NELiveStreamKit getInstance].localMember.account]) {
          if ([data.user isEqualToString:volume.userUuid]) {
            isIn = true;
            NEUIMicQueueCell *cell = [self cellWithMicOrder:i];
            if (volume.volume > 0 && member.isAudioOn) {
...
- (void)setAnchorMicInfo:(NELiveStreamSeatItem *)anchorMicInfo {
  if (!anchorMicInfo) {
    NELiveStreamRoomAnchor *anchor = NELiveStreamInnerSingleton.sharedInstance.roomInfo.anchor;
    anchorMicInfo = [[NELiveStreamSeatItem alloc] init];
...
- (void)updateAnchorGift {
  for (NELiveStreamBatchSeatUserReward *seatUserReward in _giftDatas) {
...
- (void)onConnectBtnPressedWithMicInfo:(NELiveStreamSeatItem *)micInfo {
  if (_delegate &&
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  if ([_datas count] > indexPath.row) {
    NELiveStreamSeatItem *data = _datas[indexPath.row];
    //      NELiveStreamBatchSeatUserReward *giftData = giftDatas[indexPath.row];
...
/// 更新礼物值，单独开方法 是因为多线程
- (void)updateGiftDatas:(NSMutableArray<NELiveStreamBatchSeatUserReward *> *)giftDatas {
  @synchronized(self) {
    self.giftDatas = giftDatas;
    //    for (NELiveStreamBatchSeatUserReward *batchSeatUserReward in giftDatas) {
...
- (void)updateGiftData:(NSString *)account {
  ...
  @synchronized(self) {
    ...
    for (int index = 0; index < self.giftDatas.count; index++) {
      NELiveStreamBatchSeatUserReward *batchSeatUserReward = self.giftDatas[index];
