...
@interface NELiveStreamGiftToCell ()
...
@end
@implementation NELiveStreamGiftToCell
...
+ (NELiveStreamGiftToCell *)cellWithCollectionView:(UICollectionView *)collectionView
  ...
  NELiveStreamGiftToCell *cell =
      [collectionView dequeueReusableCellWithReuseIdentifier:[NELiveStreamGiftToCell description]
                                                forIndexPath:indexPath];
  if ([[NELiveStreamGiftEngine getInstance].selectedSeatDatas
  ...
  cell.icon.image = [NELiveStreamUI ne_livestream_imageName:@"default_seat_icon"];
  if (indexPath.row == 0) {
...
- (void)installWithModel:(NELiveStreamSeatItem *)model {
  if (model.icon.length > 0) {
    [self.icon sd_setImageWithURL:[NSURL URLWithString:model.icon]
                 placeholderImage:[NELiveStreamUI ne_livestream_imageName:@"default_seat_icon"]];
  }
}
