...
#import "NELiveStreamSendGiftViewController.h"
#import <Masonry/Masonry.h>
#import <NEUIKit/UIImage+NEUIExtension.h>
#import "NELiveStreamGiftEngine.h"
#import "NELiveStreamGiftNumCell.h"
#import "NELiveStreamGiftToCell.h"
...
#import "NELiveStreamPopover.h"
#import "NELiveStreamSendGiftCell.h"
#import "NELiveStreamToast.h"
#import "NELiveStreamUIActionSheetNavigationController.h"
#import "NELiveStreamUIKit/NELiveStreamUIKit-Swift.h"

@interface NELiveStreamSendGiftViewController () <UICollectionViewDelegate,
                                                  UICollectionViewDataSource,
                                                  UITableViewDataSource,
                                                  UITableViewDelegate>

/// 代理
@property(nonatomic, weak) id<NELiveStreamSendGiftViewtDelegate> giftDelegate;
/// 礼物展示视图
...
/// 礼物数组
@property(nonatomic, strong) NSArray<NELiveStreamUIGiftModel *> *gifts;
/// 礼物个数数组
...
/// 选中的礼物
@property(nonatomic, strong) NELiveStreamUIGiftModel *selectedGift;
...
@property(nonatomic, strong) NELiveStreamPopover *btnPopover;

@end

@implementation NELiveStreamSendGiftViewController
...
+ (NELiveStreamSendGiftViewController *)showWithTarget:(id<NELiveStreamSendGiftViewtDelegate>)target
  ...
  NELiveStreamSendGiftViewController *vc = [[NELiveStreamSendGiftViewController alloc] init];
...
- (instancetype)init {
  if ([super init]) {
    self.gifts = [NELiveStreamUIGiftModel defaultGifts];
...
- (void)viewDidLoad {
  ...
  [NELiveStreamUILog infoLog:@"GetSeatInfo" desc:[NSString stringWithFormat:@"%s", __FUNCTION__]];
  [self getSeatInfo];
...
- (void)setDatas:(NSArray<NELiveStreamSeatItem *> *)datas {
  _datas = datas;
...
- (void)setAnchorMicInfo:(NELiveStreamSeatItem *)anchorMicInfo {
...
- (void)getSeatInfo {
  [NELiveStreamUILog infoLog:@"GetSeatInfo" desc:[NSString stringWithFormat:@"%s", __FUNCTION__]];
  [NELiveStreamKit.getInstance getSeatInfo:^(NSInteger code, NSString *_Nullable msg,
                                             NELiveStreamSeatInfo *_Nullable seatInfo) {
    if (code == 0 && seatInfo) {
      dispatch_async(dispatch_get_main_queue(), ^{
        self.anchorMicInfo =
            [NELiveStreamInnerSingleton.sharedInstance fetchAnchorItem:seatInfo.seatItems];
        if (!self.datas) {
          self.datas =
              [NELiveStreamInnerSingleton.sharedInstance fetchAudienceSeatItems:seatInfo.seatItems];
        }
        [self.giftToCollectionView reloadData];
      });
    }
...
- (UIButton *)bottomSendGiftIcon {
  if (!_bottomSendGiftIcon) {
    ...
    [_bottomSendGiftIcon setImage:[NELiveStreamUI ne_livestream_imageName:@"send_top_icon"]
                         forState:UIControlStateNormal];
    [_bottomSendGiftIcon setImage:[NELiveStreamUI ne_livestream_imageName:@"send_bottom_icon"]
...
- (UICollectionView *)giftToCollectionView {
  if (!_giftToCollectionView) {
    ...
    layout.itemSize = [NELiveStreamGiftToCell size];
    ...
    [_giftToCollectionView registerClass:[NELiveStreamGiftToCell class]
              forCellWithReuseIdentifier:[NELiveStreamGiftToCell description]];
...
- (UIImageView *)giftNumImageView {
  if (!_giftNumImageView) {
    _giftNumImageView = [[UIImageView alloc]
        initWithImage:[NELiveStreamUI ne_livestream_imageName:@"gift_bottom_icon"]];
  }
...
- (UICollectionView *)collectionView {
  if (!_collectionView) {
    UICollectionViewFlowLayout *layout = [[UICollectionViewFlowLayout alloc] init];
    layout.itemSize = [NELiveStreamSendGiftCell size];
    layout.scrollDirection = UICollectionViewScrollDirectionHorizontal;
    ...
    [_collectionView registerClass:[NELiveStreamSendGiftCell class]
        forCellWithReuseIdentifier:[NELiveStreamSendGiftCell description]];
...
- (UITableView *)giftNumTableView {
  if (!_giftNumTableView) {
    ...
    [_giftNumTableView registerClass:[NELiveStreamGiftNumCell class]
              forCellReuseIdentifier:NELiveStreamGiftNumCell.description];
...
- (NELiveStreamPopover *)btnPopover {
  if (!_btnPopover) {
    NELiveStreamPopoverOption *option = [[NELiveStreamPopoverOption alloc] init];
    option.autoAjustDirection = YES;
    option.preferedType = NELiveStreamPopoverTypeUp;
    ...
    _btnPopover = [[NELiveStreamPopover alloc] initWithOption:option];
  }
  return _btnPopover;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  if (collectionView == self.collectionView) {
    NELiveStreamSendGiftCell *cell = [NELiveStreamSendGiftCell cellWithCollectionView:collectionView
                                                                            indexPath:indexPath
                                                                                datas:_gifts];
    return cell;
  } else {
    NELiveStreamGiftToCell *cell = [NELiveStreamGiftToCell cellWithCollectionView:collectionView
                                                                        indexPath:indexPath
  ...
  if (collectionView == self.collectionView) {
    ...
    if (indexPath.row == 0) {
      ...
      if ([self.datas count] + 1 > indexPath.row) {
        NELiveStreamSeatItem *seat = self.datas[indexPath.row - 1];
        if (seat.user.length <= 0 && ![[NELiveStreamGiftEngine getInstance].selectedSeatDatas
                                         containsObject:[NSNumber numberWithLong:indexPath.row]]) {
          /// 针对case 麦上无人 并且未选中该麦位，直接返回
          return;
        } else if (seat.user.length > 0 &&
                   ![[NELiveStreamGiftEngine getInstance].selectedSeatDatas
                       containsObject:[NSNumber numberWithLong:indexPath.row]] &&
                   (seat.status != NELiveStreamSeatItemStatusTaken)) {
    ...
    [[NELiveStreamGiftEngine getInstance] updateSelectedSeatDatas:indexPath.row];
    NSLog(@"选中数据 --- %@", [NELiveStreamGiftEngine getInstance].selectedSeatDatas);
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  NELiveStreamGiftNumCell *cell =
      [NELiveStreamGiftNumCell cellWithTableView:tableView
                                       indexPath:indexPath
...
- (void)sendAction:(UIButton *)sender {
  if (!_selectedGift || [NELiveStreamGiftEngine getInstance].selectedSeatDatas.count <= 0) {
    [NELiveStreamToast showToast:NELocalizedString(@"未选择送礼对象，无法送礼")];
    return;
  }

  NSMutableArray *userUuids = [NSMutableArray array];
  for (NSNumber *index in [NELiveStreamGiftEngine getInstance].selectedSeatDatas) {
    if (index.intValue == 0) {
      ...
      NELiveStreamSeatItem *seat = self.datas[[index intValue] - 1];
