...
@interface NELiveStreamSendGiftCell ()
...
@implementation NELiveStreamSendGiftCell
...
- (void)installWithModel:(NELiveStreamUIGiftModel *)model {
  self.icon.image = [NELiveStreamUI ne_livestream_imageName:model.icon];
  self.info.attributedText = [self descriptionWithGift:model];
  self.info.textAlignment = NSTextAlignmentCenter;
}

- (NSAttributedString *)descriptionWithGift:(NELiveStreamUIGiftModel *)gift {
...
- (NSAttributedString *)descriptionWithGasdift:(NELiveStreamUIGiftModel *)gift {
  NSMutableParagraphStyle *style = [[NSMutableParagraphStyle alloc] init];
  style.minimumLineHeight = 20;
  style.maximumLineHeight = 20;
...
+ (NELiveStreamSendGiftCell *)cellWithCollectionView:(UICollectionView *)collectionView
  ...
  NELiveStreamSendGiftCell *cell =
      [collectionView dequeueReusableCellWithReuseIdentifier:[NELiveStreamSendGiftCell description]
                                                forIndexPath:indexPath];
  if ([datas count] > indexPath.row) {
    NELiveStreamUIGiftModel *gift = datas[indexPath.row];
