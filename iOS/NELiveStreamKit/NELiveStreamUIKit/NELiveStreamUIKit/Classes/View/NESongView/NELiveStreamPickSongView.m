...
#import "NELiveStreamPickSongView.h"
...
#import "NELiveStreamPointSongTableViewCell.h"
#import "NELiveStreamPointedSongTableViewCell.h"
#import "NELiveStreamSongEmptyView.h"
#import "NELiveStreamSongItem.h"
#import "NELiveStreamSongPlayControlView.h"
#import "NELiveStreamToast.h"
// #import "NEVoiceRoomUI.h"
// #import "NELiveStreamUILog.h"
// #import "NTESGlobalMacro.h"
// #import <NELiveStreamUIKit/NELiveStreamUIKit-Swift.h>

#import "NELiveStreamUIKit/NELiveStreamUIKit-Swift.h"

@interface NELiveStreamPickSongView () <UITableViewDelegate,
                                        UITableViewDataSource,
                                        NESongPreloadProtocol,
                                        NESongPointProtocol,
                                        UITextFieldDelegate,
                                        NELiveStreamSongPlayControlViewDelegate>
// 顶部切换视图
...
@property(nonatomic, strong) NELiveStreamRoomInfo *detail;

@property(nonatomic, strong) NELiveStreamSongEmptyView *emptyView;

@property(nonatomic, strong) NELiveStreamSongPlayControlView *playControlView;

// 当前点歌数据：用于麦位申请的时候
@property(nonatomic, strong) NELiveStreamSongItem *currentOrderSong;

@end
@implementation NELiveStreamPickSongView

- (instancetype)initWithFrame:(CGRect)frame detail:(NELiveStreamRoomInfo *)detail {
  ...
  if (self) {
    ...
    [[NELiveStreamPickSongEngine sharedInstance] addObserve:self];
  }
  return self;
}

- (void)refreshData {
  [[NELiveStreamPickSongEngine sharedInstance] updateSongArray];
  __weak typeof(self) weakSelf = self;
  [[NELiveStreamPickSongEngine sharedInstance] getKaraokeSongList:^(NSError *_Nullable error) {
    if (error) {
      [NELiveStreamToast showToast:NELocalizedString(@"获取歌曲列表失败")];
    } else {
      dispatch_async(dispatch_get_main_queue(), ^{
        [weakSelf.pickSongsTableView reloadData];
        [[NELiveStreamPickSongEngine sharedInstance] updatePageNumber:NO];
      });
    }
  }];
  [[NELiveStreamPickSongEngine sharedInstance]
      getKaraokeSongOrderedList:^(NSError *_Nullable error) {
        if (error) {
          [NELiveStreamToast showToast:NELocalizedString(@"获取已点列表失败")];
        } else {
          dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (!weakSelf.pointButtonSelected) {
              weakSelf.emptyView.hidden =
                  [NELiveStreamPickSongEngine sharedInstance].pickedSongArray.count;
            }
...
- (void)refreshPickedSongView {
  ...
  [[NEOrderSong getInstance] queryPlayingSongInfo:^(NSInteger code, NSString *_Nullable msg,
    ...
    if (code == NEOrderSongErrorCode.success) {
      ...
      [NELiveStreamPickSongEngine sharedInstance].currrentSongModel = model;
      /// 获取已点列表数据
      [[NELiveStreamPickSongEngine sharedInstance]
          getKaraokeSongOrderedList:^(NSError *_Nullable error) {
            if (error == nil) {
...
- (void)initPickSongView {
  ...
  self.searchImageView =
      [[UIImageView alloc] initWithImage:[NELiveStreamUI ne_livestream_imageName:@"icon_search"]];
  ...
  [self.searchClearButton
      setBackgroundImage:[NELiveStreamUI ne_livestream_imageName:@"icon_search_cancel"]
                forState:UIControlStateNormal];
  ...
  [self.pickSongsTableView registerClass:[NELiveStreamPointSongTableViewCell class]
  ...
  [self.pickedSongsTableView registerClass:[NELiveStreamPointedSongTableViewCell class]
  ...
  self.emptyView = [[NELiveStreamSongEmptyView alloc] init];
  ...
  self.pickSongsTableView.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
    if ([NELiveStreamPickSongEngine sharedInstance].noMore) {
      [NELiveStreamToast showToast:NELocalizedString(@"无更多内容")];
...
- (void)refreshList {
  [[NELiveStreamPickSongEngine sharedInstance] updateSongArray];
  if (self.isSearching) {
    [NELiveStreamPickSongEngine sharedInstance].searchPageNum = 0;
    [self getKaraokeSearchSongsList];
  } else {
    [NELiveStreamPickSongEngine sharedInstance].pageNum = 0;
...
- (void)clickPickedButton:(UIButton *)sender {
  ...
  self.playControlView.hidden = ![NELiveStreamPickSongEngine sharedInstance].pickedSongArray.count;
  self.emptyView.hidden = [NELiveStreamPickSongEngine sharedInstance].pickedSongArray.count;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
  ...
  if (tableView == self.pickSongsTableView) {
    return [NELiveStreamPickSongEngine sharedInstance].pickSongArray.count;
  } else {
    return [NELiveStreamPickSongEngine sharedInstance].pickedSongArray.count;
  }
}
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  if (tableView == self.pickSongsTableView) {
    // 歌曲列表页面
    NELiveStreamPointSongTableViewCell *cell =
        [tableView dequeueReusableCellWithIdentifier:@"Identifier" forIndexPath:indexPath];
    if ([NELiveStreamPickSongEngine sharedInstance].pickSongArray.count <= indexPath.row) {
      return cell;
    }
    NELiveStreamSongItem *item =
        [NELiveStreamPickSongEngine sharedInstance].pickSongArray[indexPath.row];
    NSString *downlaodingStatus =
        [NELiveStreamPickSongEngine sharedInstance].pickSongDownloadingArray[indexPath.row];
    if (item.songCover.length > 0) {
      [cell.songImageView sd_setImageWithURL:[NSURL URLWithString:item.songCover]];
    } else {
      cell.songImageView.image = [NELiveStreamUI ne_livestream_imageName:@"empty_song_cover"];
    }
    ...
    if (item.channel == CLOUD_MUSIC) {
      cell.resourceImageView.image =
          [NELiveStreamUI ne_livestream_imageName:@"pointsong_clouldmusic"];
    } else if (item.channel == MIGU) {
      cell.resourceImageView.image = [NELiveStreamUI ne_livestream_imageName:@"pointsong_migu"];
    } else {
    ...
    cell.clickPointButton = ^{
      ...
      [NELiveStreamUILog successLog:liveStreamUILog desc:logInfo];
      {
        ...
        [NELiveStreamUILog successLog:liveStreamUILog desc:viewLogInfo];
        [[NELiveStreamPickSongEngine sharedInstance].currentOrderSongArray addObject:item];
        NSString *downloadingLogInfo =
            [NSString stringWithFormat:@"点击开始下载文件,下载中列表数据变更:%@", item.songId];
        [NELiveStreamUILog successLog:liveStreamUILog desc:downloadingLogInfo];
        [[NELiveStreamPickSongEngine sharedInstance] preloadSong:item.songId channel:item.channel];
      }
    };
    return cell;

  } else {
    NELiveStreamPointedSongTableViewCell *cell =
        [tableView dequeueReusableCellWithIdentifier:@"Identifier2" forIndexPath:indexPath];
    if ([NELiveStreamPickSongEngine sharedInstance].pickedSongArray.count <= indexPath.row) {
      return cell;
    }
    NEOrderSongResponse *item =
        [NELiveStreamPickSongEngine sharedInstance].pickedSongArray[indexPath.row];
    if ([[NELiveStreamPickSongEngine sharedInstance].currrentSongModel.playMusicInfo.songId
            isEqualToString:item.orderSong.songId] &&
        [NELiveStreamPickSongEngine sharedInstance].currrentSongModel.playMusicInfo.oc_channel ==
      ...
      if ([self.detail.anchor.userUuid
              isEqualToString:[NELiveStreamKit getInstance].localMember.account]) {
        // 是主播
    ...
    if (item.orderSongUser.icon) {
      ...
      [cell.userIconImageView
          setImage:[NELiveStreamUI ne_livestream_imageName:@"user_default_icon"]];
    }
...
- (void)applySuccess {
  ...
  //                  complete:^{
  //                    [[NELiveStreamPickSongEngine sharedInstance].currentOrderSongArray
...
- (void)onSourceReloadIndex:(NSIndexPath *)index process:(float)progress {
  dispatch_async(dispatch_get_main_queue(), ^{
    if ([NELiveStreamPickSongEngine sharedInstance].pickSongArray.count > index.row) {
      NELiveStreamPointSongTableViewCell *cell =
...
- (void)onOrderSong:(NEOrderSongResponse *)songModel error:(NSString *)errorMessage {
  if (errorMessage && errorMessage.length > 0) {
    [NELiveStreamToast showToast:errorMessage];
  }
}
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
  ...
  [[NELiveStreamPickSongEngine sharedInstance] resetPageNumber];
  [[NELiveStreamPickSongEngine sharedInstance] updateSongArray];
  [self getKaraokeSearchSongsList];
  return YES;
}
                      ...
                      callback:^(NSError *_Nullable error) {
                        dispatch_async(dispatch_get_main_queue(), ^{
                          ...
                          if (error) {
                            ...
                            [weakSelf.pickSongsTableView reloadData];
                            [NELiveStreamToast showToast:NELocalizedString(@"没有找到合适的结果")];
                          } else {
                            [[NELiveStreamPickSongEngine sharedInstance] updatePageNumber:YES];
                            [weakSelf.pickSongsTableView reloadData];
                            if ([weakSelf.pickSongsTableView.refreshControl isRefreshing]) {
                              [weakSelf.pickSongsTableView.refreshControl endRefreshing];
                            }
                            if ([[NELiveStreamPickSongEngine sharedInstance] pickSongArray].count <=
                                0) {
                              [NELiveStreamToast
...
- (void)onVoiceRoomSongTokenExpired {
  [NELiveStreamToast showToast:NELocalizedString(@"版权token过期，请稍后再试")];
}
...
#pragma mark---- Getter
- (NELiveStreamSongPlayControlView *)playControlView {
  if (!_playControlView) {
    _playControlView = [[NELiveStreamSongPlayControlView alloc] init];
