...
@implementation NELiveStreamPopover

- (instancetype)init {
  if (self = [super init]) {
    self.option = [[NELiveStreamPopoverOption alloc] init];
...
- (void)show:(UIView *)contentView fromView:(UIView *)fromView inView:(UIView *)inView {
  if (_option.autoAjustDirection) {
    CGPoint downPoint = [self arrowPointWithView:contentView
                                        fromView:fromView
                                          inView:inView
                                     popoverType:NELiveStreamPopoverTypeDown];
    CGPoint upPoint = [self arrowPointWithView:contentView
                                      fromView:fromView
                                        inView:inView
                                   popoverType:NELiveStreamPopoverTypeUp];
    ...
    if (canBeUp && !canBeDown) {
      _option.popoverType = NELiveStreamPopoverTypeUp;
    } else if (!canBeUp && canBeDown) {
      _option.popoverType = NELiveStreamPopoverTypeDown;
    } else {
...
- (CGPoint)arrowPointWithView:(UIView *)contentView
  ...
  switch (type) {
    case NELiveStreamPopoverTypeUp: {
      point = [inView
    ...
    case NELiveStreamPopoverTypeDown: {
      point = [inView
          convertPoint:CGPointMake(fromView.frame.origin.x + (fromView.frame.size.width / 2),
