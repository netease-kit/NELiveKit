...
extension NPTHomeViewController: UITableViewDelegate {
  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    if indexPath.section == 0 {
      let vc = NELiveStreamRoomListViewController()
...
extension NPTHomeViewController: UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    if let cell = tableView.dequeueReusableCell(withIdentifier: "NPTHomeCellIdentifier") as? NPTHomeCell {
      ...
      if indexPath.section == 0 {
        cell.backgroundImage.image = UIImage(named: "home_light_blue")
        cell.lottieView.animation = LottieAnimation.named("liveStream")
        cell.typeLabel.text = "LiveStream".localized
        cell.descriptionLabel.text = "LiveStream_Description".localized
