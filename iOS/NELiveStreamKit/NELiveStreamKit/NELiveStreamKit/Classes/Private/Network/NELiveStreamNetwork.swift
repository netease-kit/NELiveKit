...
let NE = NELiveStreamNetwork.shared

/// 网络请求类
class NELiveStreamNetwork {
  let tag: String = "NELiveStreamNetwork"
  static let shared = NELiveStreamNetwork()
  // 配置信息
  ...
  func request(_ url: String,
    ...
    NELiveStreamLog.networkLog(
      tag,
      desc: "HTTPRequest\nUrl: \(url).\nMethod: \(method)\nHeaders: \(headers)\nBody: \(body?.prettyJSON ?? "")"
    )
    guard let URL = URL(string: url) else {
      NELiveStreamLog.errorLog(tag, desc: "\(url). Bad Url.")
      failed(makeError(NELiveStreamErrorCode.failed, "Bad url"))
      return
    }
    ...
    let dataTask = session.dataTask(with: request) { [weak self] data, response, error in
      ...
      guard httpResp.statusCode == 200 else {
        ...
        failed(makeError(NELiveStreamErrorCode.failed,
                         "Http error with status: \(httpResp.statusCode)"))
        return
      }
      ...
      guard let response = try? JSONSerialization.jsonObject(
        ...
        failed(makeError(NELiveStreamErrorCode.failed, "Cannot decode content data"))
        return
      }
      // 打印requestId
      if let requestId = response["requestId"] as? String {
        NELiveStreamLog.infoLog(self.tag, desc: "Url:\(url) RequestId:\(requestId)")
      }
      guard let code = response["code"] as? Int else {
        failed(makeError(NELiveStreamErrorCode.failed, "Empty code in response body!"))
        return
      }
      guard code == 200 else {
      ...
      NELiveStreamLog.infoLog(self.tag, desc: "Response data:\n\(response.prettyJSON)")
      success(dic)
    }
    dataTask.resume()
  }
  ...
  func request<T>(_ url: String,
    ...
    request(url, method: method, headers: headers, body: body) { dic in
      ...
      success(NELiveStreamDecoder.decode(returnType, param: dic) ?? nil)
    } failed: { error in
      failed(error)
    }
  }
