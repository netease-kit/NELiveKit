...
//  NELiveStreamDemo
//
//  Created by Simon Blue  on 2025/5/12.
//

import UIKit
import NELiveStreamUIKit
import NEOrderSong
...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
  ...
  func checkFirstRun() {
    ...
    lazy var loginView:NELoginSampleView = {
        ...
        loginView.setIcon(UIImage(named: "liveStream_login") ?? UIImage())
...
extension AppDelegate {
  ...
  func initAllModules(_ appKey: String, extra: [String: String] = [String: String](), callback: @escaping (Int, String?) -> Void) {
    ...
    func checkCallback() {
      ...
      initLiveStreamKit(appKey, extras: extra) { code, msg, _ in
      if code != 0 {
        callback(code, msg)
      } else {
        initQueue.async {
  ...
  func initLiveStreamKit(_ appKey: String, extras: [String: String] = [String: String](), callback: @escaping (Int, String?, Any?) -> Void) {
    let config = NELiveStreamKitConfig(appKey: appKey)
    config.appKey = appKey
    var liveStreamExtras = extras
    liveStreamExtras["baseUrl"] = Configs.liveStreamBaseUrl
    config.extras = liveStreamExtras
    NELiveStreamUIManager.shared.initialize(with: config, configId: Configs.liveStreamConfigId, callback: callback)
  }
...
extension AppDelegate {
  ...
  public func loginRoom(shouldInit: Bool = false , callback:((Bool)->Void)? = nil ) {
    ...
    func login() {
    NELiveStreamUIManager.shared.login(account: userUuid, token: userToken, nickname: userName) {  code, msg, _ in
        if code == 0 {
...
/// 被踢监听
extension AppDelegate: NELiveStreamUIDelegate {
